...
//            float x = 0.001F;
        ...
        if (rUf && !isFaceOccludedByState(region, Math.min(Math.min(nwHeight, swHeight), Math.min(seHeight, neHeight)), Direction.UP, blockPos, upState)) {
            ...
            putQuad(modelQuad, bufferBuilder, x0, y0, z0, false, fluidSurfaceFlags);

            if (fluidState.shouldRenderBackwardUpFace(region, blockPos.above())) {
                putQuad(modelQuad, bufferBuilder, x0, y0, z0, true, fluidSurfaceFlags);
            }
        ...
        if (rDf) {
            ...
            putQuad(modelQuad, bufferBuilder, x0, y0, z0, false, fluidMaterialFlags);
        }
        ...
        for (Direction direction : Util.XZ_DIRECTIONS) {
            ...
            if (!isOverlay) {
                putQuad(modelQuad, bufferBuilder, x0, y0, z0, true, fluidMaterialFlags);
            }
    ...
    private void putQuad(ModelQuad quad, TerrainBufferBuilder bufferBuilder, float xOffset, float yOffset, float zOffset, boolean flip, int materialFlags) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            bufferBuilder.vertex(x, y, z, this.quadColors[i], quad.getU(i), quad.getV(i), quadLightData.lm[i] | materialFlags, 0);
