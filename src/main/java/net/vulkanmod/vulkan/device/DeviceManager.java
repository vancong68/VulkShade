...
//                deviceVulkan13Features.pNext(deviceVulkan11Features.address());
            ...
            createInfo.ppEnabledExtensionNames(asPointerBuffer(getEnabledDeviceExtensions(stack, physicalDevice)));
...
//            Configuration.DEBUG_FUNCTIONS.set(true);
    ...
    private static List<String> getEnabledDeviceExtensions(MemoryStack stack, VkPhysicalDevice device) {
        var enabledExtensions = new ArrayList<>(Vulkan.REQUIRED_EXTENSION);
        var availableExtensions = getAvailableExtension(stack, device);

        boolean hasPortabilitySubset = availableExtensions.stream()
                .map(VkExtensionProperties::extensionNameString)
                .anyMatch(PORTABILITY_SUBSET_EXTENSION::equals);

        if (hasPortabilitySubset) {
            enabledExtensions.add(PORTABILITY_SUBSET_EXTENSION);
        }
